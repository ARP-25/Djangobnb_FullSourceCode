...
const ConversationDetail: React.FC<ConversationDetailProps> = ({ userId, token, messages, conversation }) => {
        ...
        <>
            <div ref={messagesDiv} className="max-h-[400px] overflow-auto flex flex-col space-y-4">
                ...
                {realtimeMessages.map((message, index) => (
                    <div key={index} className={`w-[80%]py-4 px-6 rounded-xl ${message.name == myUser?.name ? "ml-[20%] bg-blue-200" : "bg-gray-200"}`}>
            ...
            <div className="mt-4 py-4 px-6 border border-gray-300 rounded-xl">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    />
                    <CustomButton label="Send" onClick={sendMessage} className="w-full" />
                </div>
            </div>
        </>
    );
